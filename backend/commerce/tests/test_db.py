...
def test_db_connection_and_query(tmp_path):
    ...
    os.environ["COMMERCE_DATABASE_URL"] = f"sqlite:///{db_path}"

    import importlib
    db_session = importlib.import_module("commerce.app.db.session")
    ...
    with db_session.engine.begin() as conn:
        ...
        conn.execute(text("INSERT INTO test_table (name) VALUES ('alice')"))
...
def test_alembic_upgrade_and_downgrade(tmp_path):
    db_path = tmp_path / "test_commerce_migrations.db"
    os.environ["COMMERCE_DATABASE_URL"] = f"sqlite:///{db_path}"
